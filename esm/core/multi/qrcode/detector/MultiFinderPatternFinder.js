...
var MultiFinderPatternFinder = /** @class */ (function (_super) {
        ...
        for (var i1 = 0; i1 < (size - 2); i1++) {
            ...
            for (var i2 = i1 + 1; i2 < (size - 1); i2++) {
                ...
                for (var i3 = i2 + 1; i3 < size; i3++) {
                    ...
                    var test_1 = [p1, p2, p3];
                    ResultPoint.orderBestPatterns(test_1);
                    // Calculate the distances: a = topleft-bottomleft, b=topleft-topright, c = diagonal
                    var info = new FinderPatternInfo(test_1);
                    var dA = ResultPoint.distance(info.getTopLeft(), info.getBottomLeft());
                    var dC = ResultPoint.distance(info.getTopRight(), info.getBottomLeft());
                    var dB = ResultPoint.distance(info.getTopLeft(), info.getTopRight());
                    // Check the sizes
    ...
    MultiFinderPatternFinder.prototype.findMulti = function (hints) {
        var e_1, _a;
        var tryHarder = hints != null && hints.has(DecodeHintType.TRY_HARDER);
