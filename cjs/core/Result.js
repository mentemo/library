...
var BarcodeFormaHelpers_1 = require("./util/BarcodeFormaHelpers");
/**
...
var Result = /** @class */ (function () {
    function Result(text, rawBytes, numBits_resultPoints, resultPoints_format, format_timestamp, timestamp) {
        // checks overloading order from most to least params
        if (format_timestamp === void 0) { format_timestamp = null; }
        if (timestamp === void 0) { timestamp = System_1.default.currentTimeMillis(); }
        this.text = text;
        this.rawBytes = rawBytes;
        this.timestamp = timestamp;
        // check overload 3
        if (numBits_resultPoints instanceof Number && Array.isArray(resultPoints_format) && BarcodeFormaHelpers_1.isBarcodeFormatValue(format_timestamp)) {
        ...
        // check overload 2
        if (Array.isArray(resultPoints_format) && BarcodeFormaHelpers_1.isBarcodeFormatValue(format_timestamp)) {
            this.constructorOverload2(text, rawBytes, resultPoints_format, format_timestamp, timestamp);
            return;
        }
        // check overload 1
        if (typeof text === 'string' && rawBytes instanceof Uint8Array && Array.isArray(numBits_resultPoints) && BarcodeFormaHelpers_1.isBarcodeFormatValue(resultPoints_format)) {
    ...
    Result.prototype.constructorOverload1 = function (text, rawBytes, resultPoints, format) {
        return this.constructorOverload2(text, rawBytes, resultPoints, format, System_1.default.currentTimeMillis());
    };
    Result.prototype.constructorOverload2 = function (text, rawBytes, resultPoints, format, timestamp /* long */) {
        return this.constructorImpl(text, rawBytes, rawBytes == null ? 0 : 8 * rawBytes.length, resultPoints, format, timestamp);
    };
    Result.prototype.constructorImpl = function (text, rawBytes, numBits, resultPoints, format, timestamp) {
        this.text = text;
        this.rawBytes = rawBytes;
